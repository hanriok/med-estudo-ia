...
const FlashcardGenerator: React.FC<FlashcardGeneratorProps> = ({ user, language, onNavigate, materials }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in pb-20">
        ...
        <div className="bg-[#F2F2F7] p-4 rounded-3xl border border-[#E5E5EA] mb-6 space-y-4">
             {materials.length > 0 && (
            ...
            <div className="bg-white p-2 rounded-3xl shadow-sm border border-[#E5E5EA] flex flex-col md:flex-row items-stretch gap-2">
                ...
                <button
                    ...
                    disabled={isLoading}
                    className="md:w-48 bg-[#007AFF] text-white font-bold rounded-2xl hover:bg-[#0062cc] disabled:bg-[#C7C7CC] disabled:cursor-not-allowed transition-all flex flex-col items-center justify-center p-4"
                >
        ...
        <div className="mt-4">
            {isLoading ? (
                <div className="h-96 flex flex-col items-center justify-center bg-white rounded-3xl border border-[#E5E5EA]">
                ...
                <div className="flex flex-col items-center">
                    <div className="w-full aspect-[3/2] perspective-1000 group">
                        ...
                        >
                            {/* Front */}
                            <div className="absolute w-full h-full backface-hidden bg-gradient-to-br from-[#007AFF] to-[#0055B3] text-white shadow-xl rounded-3xl flex flex-col items-center justify-center p-8 text-center">
                                <span className="text-xs font-bold uppercase tracking-widest opacity-50 mb-4">Question</span>
                                <p className="text-2xl font-bold leading-snug">{flashcards[currentIndex].question}</p>
                                <span className="absolute bottom-6 text-xs opacity-50 font-medium">Tap to flip</span>
                            </div>
                            
                            {/* Back */}
                            <div className="absolute w-full h-full backface-hidden bg-white border border-[#E5E5EA] shadow-xl rounded-3xl flex flex-col items-center justify-center p-8 text-center rotate-y-180">
                                <span className="text-xs font-bold text-[#8E8E93] uppercase tracking-widest mb-4">Answer</span>
                                <p className="text-xl text-[#1C1C1E] leading-relaxed">{flashcards[currentIndex].answer}</p>
                            </div>
                        </div>
                    </div>
                    
                     <div className="mt-8 flex items-center justify-between w-full max-w-md bg-[#E5E5EA] rounded-full p-1.5">
                        <button onClick={handlePrev} disabled={currentIndex === 0} className="p-3 rounded-full bg-white shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:shadow-none transition-all">
                            <ArrowLeftIcon className="w-5 h-5 text-[#1C1C1E]"/>
                        </button>
                        <div className="flex items-center gap-3">
                            <span className="text-[#8E8E93] font-medium font-mono text-sm">{currentIndex + 1} / {flashcards.length}</span>
                             <button onClick={handleSpeak} className="p-2 rounded-full hover:bg-white/50 transition-colors">
                                <Volume2Icon className={`w-5 h-5 ${isSpeaking ? 'text-[#007AFF]' : 'text-[#8E8E93]'}`} />
                            </button>
                        </div>
                        <button onClick={handleNext} disabled={currentIndex === flashcards.length - 1} className="p-3 rounded-full bg-white shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:shadow-none transition-all">
                           <ArrowRightIcon className="w-5 h-5 text-[#1C1C1E]" />
                        </button>
                    </div>
                </div>
            ) : (
                 <div className="h-96 flex flex-col items-center justify-center bg-[#F2F2F7] rounded-3xl border border-dashed border-[#C7C7CC]">
                    <LayersIcon className="w-12 h-12 text-[#C7C7CC]" />
                    <p className="mt-4 text-[#8E8E93] font-medium">{T.flashcardGenerator.placeholderCard}</p>
                </div>
            )}
        </div>
    </div>
